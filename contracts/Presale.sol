...
contract Presale is Ownable {
    struct PresaleStruct {
        ...
        mapping(address => bool) hasClaimedNonVested; 
    }
    ...
    function createPresale(
        ...
        emit PresaleCreated(
            ...
            presaleCounter, // The index of the newly created auction
            _startTime,
            _endTime
        );
    }
    ...
    function whitelistMultipleAddresses(
        ...
        for (uint i = 0; i < _buyers.length; i++) {
            presale.whitelisted[_buyers[i]] = true;
        }
    }
...
function claimTokens(uint256 _presaleId) external {
    ...
    if (presale.vestingEnabled) {
        ...
        // Non-vesting logic with claim protection
        require(!presale.hasClaimedNonVested[msg.sender], "Already claimed all tokens");
        uint256 tokensToClaim = presale.tokensPurchased[msg.sender] -
            presale.tokensVested[msg.sender];
        require(tokensToClaim > 0, "No tokens to claim");
        
        presale.hasClaimedNonVested[msg.sender] = true;  // Mark as claimed
