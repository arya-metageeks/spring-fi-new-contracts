...
contract AuctionD is Ownable {
    ...
    function createAuction(
        ...
        ERC20 token = ERC20(params.tokenAddress);
    ...
    function _handleTokenTransfer(
        ...
        if (params.devFeeInToken) {
            uint256 tokensForDevFee = (params.tokensToSell *
            ...
            token.transferFrom(
                ...
                tokensForDevFee +
                    params.tokensToSell +
                    ((params.tokensToSell * params.liquidityAdditionPercent) /
                        100)
            );
        } else {
            require(
    ...
    function _createAuctionStruct(
        ...
        newAuction.purchaseToken = ERC20(params.purchaseTokenAddress);
        newAuction.creator = msg.sender;
        newAuction.whitelistedEnabled = params.whitelistedEnabled;
        newAuction.burnOrRefund = params.burnOrRefund;
        newAuction.vestingEnabled = params.vestingEnabled;
        newAuction.devFeeInToken = params.devFeeInToken;
        newAuction.tokensToSell = params.tokensToSell;
        newAuction.softCap = softCap;
        newAuction.hardCap = hardCap;
        newAuction.startPrice = params.startPrice;
        newAuction.endPrice = params.endPrice;
        newAuction.minBuy = params.minBuy;
        newAuction.maxBuy = params.maxBuy;
        newAuction.decPriceCycle = params.decPriceCycle;
        newAuction.startTime = params.startTime;
        newAuction.endTime = params.endTime;
        newAuction.liquidityAdditionPercent = params.liquidityAdditionPercent;
        newAuction.liquidityUnlockTime = params.liquidityUnlockTime;
        newAuction.listingAmount = ((params.tokensToSell *
            params.liquidityAdditionPercent) / 100);
        newAuction.firstReleasePercentage = params.firstReleasePercentage;
        newAuction.vestingPeriod = params.vestingPeriod;
        newAuction.cycleReleasePercentage = params.cycleReleasePercentage;
    ...
    function validateAuctionParams(AuctionParams memory params) internal view {
        ...
        require(
            params.startPrice >= params.endPrice / 5,
            "Softcap must be more than 20% of hardcap"
        );
        require(params.minBuy < params.maxBuy, "Min buy !>= max buy");
        require(
        ...
        require(
            params.liquidityAdditionPercent > 50 &&
                params.liquidityAdditionPercent <= 100,
            "LiquidityAdditionPercent must be in between 51-100%"
        );
        require(
            params.liquidityUnlockTime >= 30 days,
            "liquidityUnlockTime must be >= 30 days"
        );
        require(
            params.firstReleasePercentage + params.cycleReleasePercentage <=
                100,
            "Invalid Release %"
        );
    }
    ...
    function buyToken(uint256 _auctionIndex, uint256 _amount) external payable {
        ...
        require(_amount >= auction.minBuy && _amount <= auction.maxBuy, "Invalid _amount");
        ...
        vars.currentPrice = auction.startPrice -
            (vars.currentCycle * (auction.startPrice - auction.endPrice)) /
            vars.totalCycles;

        vars.purchaseTokenDecimals = address(auction.purchaseToken) == address(0) ? 18 : auction.purchaseToken.decimals();
        ...
        require(auction.tokensSold + vars.tokensToBuy <= auction.tokensToSell, "Hard cap reached");

        _handlePayment(auction, _amount);
        _updateAuctionState(auction, vars.tokensToBuy, _amount, vars.currentPrice, _auctionIndex);
    }

    function _handlePayment(AuctionStruct storage auction, uint256 amount) private {
        if (address(auction.purchaseToken) == address(0)) {
            require(msg.value >= amount, "Not enough MATIC provided");
        } else {
            require(auction.purchaseToken.allowance(msg.sender, address(this)) >= amount, "Check the token allowance");
            auction.purchaseToken.transferFrom(msg.sender, address(this), amount);
        }
    }

    function _updateAuctionState(AuctionStruct storage auction, uint256 tokensToBuy, uint256 amount, uint256 currentPrice, uint256 auctionIndex) private {
        auction.tokensSold += tokensToBuy;
